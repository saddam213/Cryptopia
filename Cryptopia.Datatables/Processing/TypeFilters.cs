...
namespace Cryptopia.Datatables
{
	static class TypeFilters
	{
		...
		public static string DateTimeOffsetFilter(string query, string columnname, DataTablesPropertyInfo propertyInfo,
			List<object> parametersForLinqQuery)
		{
			if (query == "~") return string.Empty;
			if (query.Contains("~"))
			...
			{
				return string.Format("{1}.ToLocalTime().ToString(\"g\").{0}",
					FilterMethod(query, parametersForLinqQuery, propertyInfo.Type), columnname);
			}
		}
		...
		public static string BoolFilter(string query, string columnname, DataTablesPropertyInfo propertyInfo,
			List<object> parametersForLinqQuery)
		{
			if (query != null)
		...
		public static string StringFilter(string q, string columnname, DataTablesPropertyInfo columntype,
			List<object> parametersforlinqquery)
		{
			if (q == ".*") return "";
			if (q.StartsWith("^"))
			{
				...
				else
				{
					...
					var parameterArg = "@" + (parametersforlinqquery.Count - 1);
					return string.Format("({0} != null && {0} != \"\" && ({0} ==  {1} || {0}.StartsWith({1})))", columnname,
						parameterArg);
				}
			}
			else
			...
			List<object> parametersForLinqQuery)
		{
			...
			{
				if (val.ToString().ToLower().StartsWith(q.ToLower()))
				{
